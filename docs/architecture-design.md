...
CLAW 是一套私有部署的个人 AI 助手系统，采用 **Client-Server 架构**，在同一仓库内分为两大部分：

- **Server（`server/`）**：基于 OpenClaw 实现，部署在 Mac Mini 上，承载全部 AI 推理、对话管理、数据存储和插件执行
- **Client（`clients/`）**：桌面宠物等客户端，运行在用户的其他电脑上，作为轻量级交互入口，通过网络连接服务端
...
└──────────┼───────────────────┼───────────────────┼──────────────┘
           ...
           ▼                   ▼                   ▼
┌─────────────────────────────────────────────────────────────────┐
│          Mac Mini 服务端 — OpenClaw（server/ 目录）               │
│                                                                 │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │                    API Gateway                          │   │
...
```
CLAW/                                   # github.com/Extreme-S/CLAW
│
├── server/                             # ===== OpenClaw 服务端（部署在 Mac Mini）=====
│   ├── main.py                         # FastAPI 入口
│   ├── requirements.txt
...
┌────────────────────┐            ┌────────────────────┐
│  clients/desktop/   │───HTTP/WS──→│  server/ (OpenClaw)│
│  CLAW 桌面宠物      │←──推送─────│  FastAPI           │
│  (PyQt6 瘦客户端)   │            │                    │
└────────────────────┘            └────────────────────┘
...
在 Mac Mini 上只需部署 `server/` 目录，常驻运行，开机自启。
...
在用户的 MacBook/PC 上只需部署 `clients/desktop/` 目录，连接 Mac Mini 上的 OpenClaw。

```bash
# 1. 克隆仓库
git clone https://github.com/Extreme-S/CLAW.git
cd CLAW/clients/desktop
...
> 目录：`server/`，部署在 Mac Mini

- [ ] 搭建 `server/` 目录，FastAPI 骨架 + 认证中间件
- [ ] Chat Engine（多轮对话 + 消息持久化）
- [ ] AI Router（OpenAI + Claude）
- [ ] SQLite 存储层
- [ ] WebSocket 流式接口

### Phase 2：桌面宠物客户端对接（1 周）
> 目录：`clients/desktop/`，运行在用户其他电脑
...
| 组件 | 技术 | 理由 |
|---|---|---|
| 服务端框架 | FastAPI (`server/`, OpenClaw) | 异步、高性能、自动生成 API 文档 |
| 数据库 | SQLite | 轻量、零运维、私有部署够用 |
| 向量存储 | FAISS | 本地部署、无需额外服务 |
| 实时通信 | WebSocket | 原生支持流式推送 |
| 桌面 UI | PyQt6 (`clients/desktop/`) | 已有基础，跨平台 |
